                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            if (response.success == 1) {
                                                document.getElementById('mensaje_mal').textContent = "!Ha ocurrido un error. Seleccionar correctamente los datos.!";
                                                document.getElementById('alert_mal').style.display = "block";
                                            } else {
                                                if (response.success == 2) {
                                                    document.getElementById('mensaje_mal').textContent = "!El registro ya existe en la base de datos.!";
                                                    document.getElementById('alert_mal').style.display = "block";
                                                } else {
                                                    if (response.success == 4) {
                                                        document.getElementById('alert_bien').style.display = "block";
                                                    } else {
                                                        document.getElementById('mensaje_mal').textContent = "!Ha ocurrido un error. Intente nuevamente.!";
                                                        document.getElementById('alert_mal').style.display = "block";
                                                    }
                                                }
                                            } 
                                    ...
                                    if (!id_carrera) {
                                        ...
                                        $.get('/home/docente-materias/create/'+ id_carrera+'/nivel', function (data){
                                            ...
                                            for (var i = 0; i < data[0].duracion; i++) {
                                                ...
                                                html_select += '<option value='+ b +" @if( old("nivel") == "+b+") {{ selected }} @endif>"+ b +'</option>';
                                                b++;
                                            }
                                    ...
                                    if (!n) {
                                       $("#materia_id").html('<option value="">Seleccione Asignatura</option>');
                                        ...
                                        for (var i = 0; i < data.length; i++) {
                                            html_select += '<option value='+ data[i].id +' @if( old("materia_id") == '+data[i].id+') {{ selected }} @endif>'+ data[i].nombre +'</option>';
                                        }
